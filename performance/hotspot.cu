...
#ifdef OUTPUT_TO_BINARY
  #ifdef GPUD
//            fprintf(stderr,"%d %d %llu\n",grid_rows,grid_cols,grid_rows*grid_cols*sizeof(float));
            nvmed_recv(file, vect, grid_rows*grid_cols*sizeof(float), 0);
  #else
            int fd = open(file, O_CREAT | O_WRONLY | O_SYNC, 0666);
	    fp = fdopen(fd,"wb");
//            fprintf(stderr,"%d %d %llu\n",grid_rows,grid_cols,grid_rows*grid_cols*sizeof(float));
...
*/
            fwrite(vect, sizeof(float), grid_rows*grid_cols, fp);
//            fprintf(stderr,"Here\n");
            fflush(fp);
            fclose(fp);
            close(fd);
...
//	printf("HGProfile: CUDAMemcpyHD %d\n",((time_end.tv_sec * 1000000 + time_end.tv_usec) - (time_start.tv_sec * 1000000 + time_start.tv_usec)));
	...
	printf("Ending simulation\n");
    	gettimeofday(&time_end, NULL);
//	printf("HGProfile: CUDAKernel %d\n",((time_end.tv_sec * 1000000 + time_end.tv_usec) - (time_start.tv_sec * 1000000 + time_start.tv_usec)));
    	gettimeofday(&time_start, NULL);

    cudaMemcpy(MatrixOut, MatrixTemp[ret], sizeof(float)*size, cudaMemcpyDeviceToHost);
    	gettimeofday(&time_end, NULL);
//	printf("HGProfile: CUDAMemcpyDH %d\n",((time_end.tv_sec * 1000000 + time_end.tv_usec) - (time_start.tv_sec * 1000000 + time_start.tv_usec)));
    	gettimeofday(&time_start, NULL);

    	gettimeofday(&time_end, NULL);
//	printf("HGProfile: FileOutput %d\n",((time_end.tv_sec * 1000000 + time_end.tv_usec) - (time_start.tv_sec * 1000000 + time_start.tv_usec)));
